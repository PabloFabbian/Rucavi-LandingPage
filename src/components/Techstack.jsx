...
function Techstack() {
    ...
    return (
        <div className="pb-6 md:pb-20" id="TechStack">
            ...
            <div
                className="TechnologiesContainer rounded-b-3xl rounded-tr-none md:rounded-tr-3xl bg-[#EBE5FF]
        mx-4 sm:mx-8 px-0 md:px-[10px] py-0 md:py-16"
            >
                <div className="carousel-wrapper pb-6 pt-4">
                    <div className="carousel">
                        {tecnologias.map((tech, index) => (
                            <TechCard key={index} tech={tech} isMobile={isMobile} />
                        ))}
                        {tecnologias.map((tech, index) => (
                            <TechCard key={index + tecnologias.length} tech={tech} isMobile={isMobile} />
                        ))}
                    </div>
                </div>
            </div>
